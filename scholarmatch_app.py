...
def load_scholarships():
    ...
    try:
        ...
        st.error("⚠️ Scholarship dataset not found. Please ensure 'scholarships_final_ready.csv' is in the same directory.")
        return None
...
def analytics_dashboard():
    ...
    with col4:
        applied_count = len(feedback_df[feedback_df['feedback_action'] == '📝 Applied/Will apply'])
    ...
    st.subheader("🏆 Top Scholarships by Feedback")
    ...
    st.subheader("⚠️ Common 'Not Eligible' Reasons")
    
    not_eligible = feedback_df[feedback_df['feedback_action'] == '❌ Not eligible']
...
def landing_page():
    ...
    col1, col2, col3 = st.columns([1, 1, 1])
    with col2:
        if st.button("📊 View Analytics (Admin)", use_container_width=True):
            st.session_state.show_analytics = True
            st.rerun()
    
    st.markdown("---")
    st.caption("💡 Your data is collected anonymously to improve recommendations.")
...
def results_page():
    ...
    with st.spinner('🔍 Analyzing 11,000+ scholarships...'):
        recommendations = recommend_scholarships(student, scholarships_df, top_k=10)
    
    # Display results
    st.title("🎯 Your Top Scholarship Matches")
    ...
    if len(recommendations) > 0:
        st.success(f"✅ Found {len(recommendations)} scholarships you're eligible for!")
    else:
        st.warning("⚠️ No scholarships found matching your profile. Try adjusting your criteria.")
    ...
    for idx, row in recommendations.iterrows():
        ...
        with st.expander(
            f"**{rank}. {row['scholarship_title']}** — Final Score: {row['final_score']:.1f}/100",
            expanded=(rank <= 3)
        ):
            ...
            with col1:
                ...
                **💰 Funding:** {row['funding_amount']} ({row['funding_category']})  
                **📅 Deadline:** {row['deadline']} [{row['deadline_urgency']}]
                
                **✅ Why You Match:**  
                {row['reasons']}
                
                **🔗 [View Full Scholarship Details]({row['link']})**
            ...
            with col2:
                ...
                if feedback_key not in st.session_state.feedback_submitted:
                    ...
                    if feedback_action == "❌ Not eligible":
